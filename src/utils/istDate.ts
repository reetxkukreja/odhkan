...
export interface ISTFormattedDate {
  ...
  displayTitle: string; // e.g. "Saturday · 3:00 PM"
  fullDisplay: string; // e.g. "Saturday, 19 September 2026 · 3:00 PM IST"
  dateInput: string; // e.g. "2026-09-19" (YYYY-MM-DD for HTML input)
  timeInput: string; // e.g. "15:00" (HH:mm for HTML input)
  hours: number;
  minutes: number;
}
...
export function istToUtcIso(dateInput: unknown, timeInput?: unknown): string {
  ...
  if (!dateParts) {
    // If no valid date provided, use today in IST
    const now = new Date();
...
export function parseISTDate(input: unknown): ISTFormattedDate {
  ...
  // Display formatting via Intl in Asia/Kolkata timezone
  ...
  // Form input formatting (en-CA provides ISO numeric year-month-day)
  const parts = new Intl.DateTimeFormat('en-CA', {
    timeZone: 'Asia/Kolkata',
    year: 'numeric',
    month: '2-digit',
...
/**
 * Format any date input into Indian standard representation e.g. "19 September 2026" or "19-09-2026"
 */
export function formatIndianDate(input: unknown): string {
  const parsed = parseISTDate(input);
